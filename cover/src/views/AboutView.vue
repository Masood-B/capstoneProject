<template>
    <div>
        <div class="container">
            <h2>My Story</h2>
            <div class="row">
                <div class="col-6">
                    <p>Welcome to Green Out, your one-stop shop for manga fans all over the country! Our story is based on a simple yet deep idea: to make the world of manga more accessible and cheap for fans in our local community. Tired of seeing fans struggle with exorbitant shipping rates and lengthy wait times to obtain their favorite manga titles from overseas, we decided to bring the manga experience to them. Green Out is on a mission to reduce the inconveniences of international shipping by curating a large selection of manga titles that you can easily enjoy without having to worry about those pesky shipping expenses.</p>
                    <p>We are passionate about manga, and our team works hard to keep our shelves loaded with the latest releases, classic favorites, and niche gems that cater to all tastes and interests. Whether you're an experienced manga collector or a curious newbie, you'll find something to pique your interest and take you to fascinating realms.</p>
                </div>
                <div class="col-6">
                    <img src="https://i.postimg.cc/3rZ0H0D8/profile-Image.png" alt="Default">
                </div>
            </div>
            <div class="row">
                <div class="col-6">
                    <img src="https://i.postimg.cc/3rZ0H0D8/profile-Image.png" alt="Default">
                </div>
                <div class="col-6">
                    <p>Thank you for choosing Green Out as your manga haven. With your support, we're making manga more accessible and affordable for everyone in our community. So, dive into our collection, explore new horizons, and embark on countless adventures through the pages of manga. Together, we're turning a new leaf in the world of manga, one chapter at a time.</p>
                    <p>Beyond providing a convenient shopping experience, Green Out is also committed to fostering a vibrant manga community. We believe that manga is not just a form of entertainment; it's a powerful medium for storytelling, cultural exploration, and artistic expression. Our shop is a space where manga enthusiasts can connect, share recommendations, and celebrate the artistry that makes this medium so special.</p>
                </div>
            </div>
            <h2>The Founder</h2>
            <p>Hi my name is Masood Basardien, the reason i built this website is ...</p>
    </div>
</div>
</template>

<script>
    export default {
        
    }
</script>

<style scoped>
h2,p{
    color: white;
}
p{
    font-size: 1.3rem;
}


</style>